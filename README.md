...
| `/ftblh message <player> <ticks> <msg>` | Enqueues a message to be sent to the player for the given duration. If `ticks` is negative, act as a priority message. The message can be a simple string, or [JSON raw text](https://minecraft.fandom.com/wiki/Raw_JSON_text_format) | `/ftblh message @s 50 "hello!"`                  |
| `/ftblh sound <player> <ticks> <sound>` | Enqueues a sound to be played to the player, with a delay of `ticks` until the next queued message is played. If `ticks` is negative, acts as a priority message.                                                                     | `/ftblh sound @s 20 "minecraft:ui.button.click"` |
